...
const ToggleDashboard = ({ myemail, myname }) => {
    ...
    const [isMedModalOpen, setIsMedModalOpen] = useState(false); // State to control medicine modal visibility
    const [isExeModalOpen, setIsExeModalOpen] = useState(false); // State to control exercise modal visibility
    ...
    const fetchcarereceivers = async () => {
        try {
            const querySnapshot = await getDocs(
                query(collection(database, 'carereceivers'))
            );
            const arr = {};
    ...
    // console.log("carereceivers", carereceivers['nivedha@gmail.com'].name);

    // Fetch medicines from the database
    const fetchMedicine = async () => {
        try {
            const unsubscribe = await onSnapshot(collection(database, 'medicines'), where("giveremail", "==", myemail), (snapshot) => {
                const arr = [];
    ...
    const fetchExercises = async () => {
        try {
            const unsubscribe = await onSnapshot(collection(database, 'exercises'), where("giveremail", "==", myemail), (snapshot) => {
                const arr = [];
                snapshot.forEach((doc) => {
                    arr.push({ id: doc.id, ...doc.data() });
                });
    ...
    const fetchHealthData = async () => {
        try {
            const unsubscribe = await onSnapshot(collection(database, 'healthdata'), where("giveremail", "==", myemail), (snapshot) => {
                const arr = [];
                snapshot.forEach((doc) => {
                    arr.push({ id: doc.id, ...doc.data() });
                });
    ...
    // Toggle exercise modal
    const toggleExeModal = () => {
        setIsExeModalOpen(!isExeModalOpen);
    }

    // Submit new medicine to the database
    const submitMedicine = () => {
        ...
        addmed();
        toggleMedModal(); // Close medicine modal
    }

    // Submit new exercise to the database
    const submitExercise = () => {
        ...
        const addExercise = async () => {
            try {
                const docRef = await addDoc(collection(database, 'exercises'), {
                    ...
                    giveremail: myemail,
                    givername: myname,
                    recievername: carereceivers[recievermail].name, // carereceivers[recievermail].name,
                    recievermail:recievermail,
                });
                console.log("Document written with ID: ", docRef.id);
            } catch (e) {
        ...
        addExercise();
        toggleExeModal(); // Close exercise modal
    }
    ...
    const submitHealthData = () => { 
        ...
        addHealthData();
        toggleHealthModal(); // Close health modal
    }
    ...
    return (
        <div style={styles.outer}>
                        ...
                        <div style={styles.horizontalscroll}>
                            {exercises.map((exercise, index) => (
                                <div style={styles.exercisecard} key={index}>
                                    ...
                                    <div style={styles.field}>
                                        <label style={styles.label} htmlFor="text">Intensity</label>
            ...
            <div style={styles.head}>
                    ...
                    <div style={styles.name}>Health Data</div>
                </div>
